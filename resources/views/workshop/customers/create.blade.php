...
@section('title', 'Νέος Πελάτης — Συνεργείο')
@section('header-title', 'Νέος Πελάτης')
...
<h1 class="woc-page-title">Νέος Πελάτης</h1>
...
<form method="POST" action="{{ route('workshop.customers.store') }}" novalidate>
    @csrf

    <div class="woc-card">
        <div class="woc-card-title">Στοιχεία Πελάτη</div>
        <div class="woc-card-body woc-card-body--grid">

            <div class="woc-field">
                <label for="first_name" class="woc-label">Όνομα <span class="woc-req">*</span></label>
                <input type="text" id="first_name" name="first_name"
                    class="woc-input {{ $errors->has('first_name') ? 'is-invalid' : '' }}"
                    value="{{ old('first_name') }}"
                    placeholder="π.χ. Γιώργος">
                @error('first_name') <span class="woc-error">{{ $message }}</span> @enderror
            </div>

            <div class="woc-field">
                <label for="last_name" class="woc-label">Επώνυμο <span class="woc-req">*</span></label>
                <input type="text" id="last_name" name="last_name"
                    class="woc-input {{ $errors->has('last_name') ? 'is-invalid' : '' }}"
                    value="{{ old('last_name') }}"
                    placeholder="π.χ. Παπαδόπουλος">
                @error('last_name') <span class="woc-error">{{ $message }}</span> @enderror
            </div>

            <div class="woc-field">
                <label for="phone" class="woc-label">Τηλέφωνο <span class="woc-req">*</span></label>
                <input type="text" id="phone" name="phone"
                    class="woc-input {{ $errors->has('phone') ? 'is-invalid' : '' }}"
                    value="{{ old('phone') }}"
                    placeholder="π.χ. 6912345678">
                @error('phone') <span class="woc-error">{{ $message }}</span> @enderror
            </div>

            <div class="woc-field">
                <label for="email" class="woc-label">Email</label>
                <input type="email" id="email" name="email"
                    class="woc-input {{ $errors->has('email') ? 'is-invalid' : '' }}"
                    value="{{ old('email') }}"
                    placeholder="π.χ. giorgos@example.com">
                @error('email') <span class="woc-error">{{ $message }}</span> @enderror
            </div>
